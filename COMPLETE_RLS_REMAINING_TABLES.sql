-- ===================================================================
-- COMPLETE RLS FOR REMAINING 19 TABLES
-- ===================================================================
...
CREATE POLICY "billing_subscriptions_customer" ON public.billing_subscriptions
  FOR ALL TO authenticated
  USING (customer_id = auth.uid() OR public.is_team_member());
...
CREATE POLICY "invoice_line_items_via_invoice" ON public.invoice_line_items
  ...
  USING (
    EXISTS (
      ...
      AND (customer_id = auth.uid() OR public.is_team_member())
    )
  );
...
CREATE POLICY "water_districts_read_all" ON public.water_districts
  ...
  USING (true);
CREATE POLICY "water_districts_modify_team" ON public.water_districts

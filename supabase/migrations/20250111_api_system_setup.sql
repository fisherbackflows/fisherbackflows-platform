-- Migration: API System Setup for Backflow Buddy Plugin
...
--     sample_api_key TEXT;
--     sample_key_hash TEXT;
-- BEGIN
--     -- Get Fisher Backflows company ID
--     SELECT id INTO fisher_company_id FROM companies WHERE slug = 'fisher-backflows' LIMIT 1;
    
--     IF fisher_company_id IS NOT NULL THEN
--         -- Generate sample API key
--         sample_api_key := generate_api_key();
--         sample_key_hash := hash_api_key(sample_api_key);
...
--             'Development Key',
--             sample_key_hash,
--             get_api_key_preview(sample_api_key),
--             true,
--             1000
--         );
        
--         RAISE NOTICE 'Sample API key created: %', sample_api_key;
--     END IF;
-- END $$;

  ...
  return (
    <div className="min-h-screen bg-black">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 relative z-10">
        {/* Page Title */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold gradient-text mb-2">Backflow Device Management</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          <div className="glass rounded-xl p-6 card-hover">
            ...
            <p className="text-white/60 text-sm">All registered devices</p>
          </div>
          ...
          <div className="glass rounded-xl p-6 card-hover">
            ...
            <p className="text-white/60 text-sm">Tests needed within 30 days</p>
          </div>
          ...
          <div className="glass rounded-xl p-6 card-hover">
            ...
            <p className="text-white/60 text-sm">Current compliant devices</p>
          </div>
          ...
          <div className="glass rounded-xl p-6 card-hover">
            ...
            <p className="text-white/60 text-sm">Devices needing attention</p>
          </div>
        </div>
            ...
            <div className="flex items-center space-x-4">
              <div className="flex items-center space-x-2">
                ...
                >
                  ...
                  <option value="due-soon" className="bg-black">Due Soon</option>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {filteredDevices.map((device) => (
            <div key={device.id} className="glass rounded-2xl p-6 glow-blue-sm">
              <div className="flex justify-between items-start mb-4">
                <div className="flex-1">
                  <h3 className="font-bold text-xl text-white mb-1">{device.location}</h3>
                  <p className="text-white/60 text-sm mb-2">{device.address}</p>
              ...
              <div className="grid grid-cols-2 gap-4 mb-4 text-sm">
                <div className="glass-darker rounded-lg p-3">
                  <p className="text-white/50 mb-1">Device Type</p>
                  <p className="text-white/80 font-medium">{device.type}</p>
                </div>
                <div className="glass-darker rounded-lg p-3">
                  <p className="text-white/50 mb-1">Install Date</p>
                  <p className="text-white/80 font-medium">{new Date(device.installDate).toLocaleDateString()}</p>
                </div>
                <div className="glass-darker rounded-lg p-3">
                  <p className="text-white/50 mb-1">Last Test</p>
                  <p className="text-white/80 font-medium">{new Date(device.lastTestDate).toLocaleDateString()}</p>
                </div>
                <div className="glass-darker rounded-lg p-3">
                  <p className="text-white/50 mb-1">Next Test Due</p>
                  <p className="text-white/80 font-medium">{new Date(device.nextTestDate).toLocaleDateString()}</p>
                </div>
              </div>
              ...
              {device.notes && (
                <div className="glass-blue rounded-lg p-3 mb-4">
                  <p className="text-white/50 text-xs mb-1">Notes</p>
                  <p className="text-white/80 text-sm">{device.notes}</p>
                </div>
              )}

              <div className="flex space-x-2">
                <Button className="flex-1 btn-glow py-2 rounded-lg text-sm">
                ...
                <Button 
                  ...
                  className="btn-glass px-3 py-2 rounded-lg"
                >
                  <Edit3 className="h-4 w-4" />
                </Button>
                <Button 
                  onClick={() => handleDeleteDevice(device.id)}
                  className="btn-glass px-3 py-2 rounded-lg hover:bg-red-500/20"
                >
        ...
        {filteredDevices.length === 0 && (
          <div className="text-center py-12">
            <Droplet className="h-16 w-16 text-white/30 mx-auto mb-4" />
            <h3 className="text-xl font-semibold text-white/60 mb-2">No devices found</h3>
